...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Option[Tweet]): Tweet = {
    val acc2 = acc match {
      case None => elem
      case Some(tw) => if (elem.retweets > tw.retweets) elem else tw
    }

    right.mostRetweetedAcc(Some(left.mostRetweetedAcc(Some(acc2))))
//
//
//    val acc2 = if (acc == null || elem.retweets > acc.retweets) elem else acc
//    right.mostRetweetedAcc(left.mostRetweetedAcc(acc2))
  }
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => {
    apple.exists(s => tw.text.contains(s))
  })
